...
void main()
{
	...
	while (context.step())
	{
		...
		tdraw(gpass, cube, gframe, model, view, proj, spear_diffuse, spear_normal, spear_specular);
		...
		tdraw(lpass, quad, lframe, view, proj,
			...
			gframe.depth, 
			glm::normalize(glm::vec4(1,1,-1,0)), 
			glm::vec4(1,0,0,1));
		...
		for (int i = 0; i < 4; ++i)
		{
			glm::mat4 mod = glm::translate(glm::vec3(-.75f + .5*i, 0.75f, 0)) *
							glm::scale(glm::vec3(0.25f, 0.25f, 1.f));
			tdraw(post, quad, screen, gframe.colors[i], mod);
		}

		glm::mat4 mod = glm::translate(glm::vec3(-.75f, 0.25f, 0)) *
			glm::scale(glm::vec3(0.25f, 0.25f, 1.f));
		tdraw(post, quad, screen, gframe.depth, mod);

		mod = glm::translate(glm::vec3(-.25f, 0.25f, 0)) *
			glm::scale(glm::vec3(0.25f, 0.25f, 1.f));
		...
		mod = glm::translate(glm::vec3(.75f, 0.25f, 0)) *
			glm::scale(glm::vec3(0.25f, 0.25f, 1.f));
